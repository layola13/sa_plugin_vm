...
- Loads FFI providers from dependencies declared in the installed `vm` manifest and only exposes raw `dlopen` / `dlsym` when `--allow-ffi` is set.
...
- `http-client`: outbound HTTP smoke symbols and body/stream helpers.
- `http-server`: inbound HTTP request/response helpers and stream helpers.
- `deno`: Deno compatibility smoke symbols.
- `node`: Node compatibility smoke symbols, including timers and UDP/datagram loopback.
